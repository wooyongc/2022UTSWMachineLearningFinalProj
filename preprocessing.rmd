...
# Actually, let's change our output variable (MMSE.Change) to final.MMSE where final.MMSE = MMSE + MMSE.Change
df_clean <- df_clean %>% mutate(MMSE.final = MMSE + MMSE.Change) %>% select(-MMSE.Change)
df_clean %>% head()

# Minmax scale 
df_minmax <- df_clean %>% mutate(AGE=minmax(AGE), PTEDUCAT = minmax(PTEDUCAT), MMSE = minmax(MMSE))
...
## gaussian
```{r}
gauss <- glm(`MMSE.final` ~ ., data = df_clean, family = gaussian())
summary(gauss)
...
## Poisson
```{r}
pois <- glm(`MMSE.final` ~ ., data = df_clean, family = poisson())
summary(pois)
...
## Poisson - minmax
```{r}
mm_pois <- glm(`MMSE.final` ~ ., data = df_minmax, family = poisson())
summary(mm_pois)
...
## Quasi-Poisson
```{r}
qpois <- glm(`MMSE.final` ~ ., data = df_clean, family = quasipoisson())
summary(qpois)
...
```{r}
rf <- randomForest(`MMSE.final` ~ ., data = df_clean, ntrees = 500, importance = TRUE, type = 'regression')
y_pred <- predict(rf)
