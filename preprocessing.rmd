...
# Optional
df_clean <- df2 <- df_clean %>% mutate(`MMSE.Change.Filtered` = ifelse(MMSE.Change < -8, -8, MMSE.Change))
# Remove non predictive features
...
```{r}
bme <- brm(MMSE.Change ~ AGE+MMSE+PTGENDER+PTEDUCAT+APOE4+DX + (1|PTID), data = df_lme)
